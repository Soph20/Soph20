...
- **HoliPet** — AI-powered pet care assistant for LATAM, built as an all-in-one platform: food subscriptions with recommendations based on pet profile and owner preferences, vet clinic booking, services, and payments — everything centralized in one place. The conversational layer lets pet owners chat naturally in four languages (EN/ES/IT/NL) and a two-tier system routes each message to the right specialized agent — SOS, health, behavior, or general care. Tier 1 is deterministic: a multilingual distress-keyword dictionary plus embedding similarity (cosine ≥ 0.90) catches emergencies before any LLM runs. Tier 2 routes everything else through a locally hosted Llama 3 70B — keeping pet-owner data off third-party APIs and cost in check. Evaluation driven by a labeled, intent-tagged ground-truth set scored with sentence-transformers; next iteration adds per-class F1 and a confusion matrix, with SOS recall prioritized because the cost of a missed emergency is asymmetric.
