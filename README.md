...
- **[HoliPet](https://app.holipet.io/auth)** — The all-in-one pet care platform for LATAM. One app for everything: food subscriptions tailored to your pet's profile, vet clinic booking, services, and payments — no more juggling five apps for one pet. The AI layer lets owners chat naturally in four languages (EN/ES/IT/NL) and routes instantly to the right specialist — SOS, health, behavior, or general care. Emergency detection is deterministic by design: a multilingual keyword net plus embedding similarity (cosine ≥ 0.90) fires before any LLM runs, because a missed emergency isn't a UX issue. Everything else routes through a locally hosted Llama 3 70B — pet-owner data stays private, costs stay low.
